      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-green-950 via-green-900 to-emerald-800 text-white shadow-2xl">
        ...
        <div className="relative p-8 md:p-10">
          <div className="flex items-center gap-3 mb-6">
            <Link href="/matches" className="text-green-300 hover:text-white transition-colors text-sm font-medium">
              ← Partidos
            </Link>
            <span className="text-green-700">·</span>
            <span className="text-green-300 text-sm">{match.date}</span>
            {match.location && <span className="text-green-400 text-sm">· 📍 {match.location}</span>}
                  ...
                  <div className="space-y-2 text-right">
                    {opt.team2.map((p, i) => (
                      <div key={i} className="flex items-center justify-end gap-2">
                        ...
                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-red-400 to-red-600 flex items-center justify-center text-white text-xs font-black shrink-0">
                          {p.name.charAt(0)}
                        </div>
                      </div>
                    ))}
                    <p className="text-xs font-bold text-red-600 pr-10">Elo equipo: {Math.round(opt.team2Elo)}</p>
                  </div>
                </div>
              </div>
            ))}
