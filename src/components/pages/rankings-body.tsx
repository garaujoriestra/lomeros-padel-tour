...
// Body compartido entre (public)/rankings/page.tsx y g/[slug]/rankings/page.tsx
// (>80 líneas → extracción, criterio del plan de paridad). `ctx.groupId` sustituye
// a `getDefaultGroupId()`, `ctx.player` a la llamada interna a `resolvePageContext()`,
// y `ctx.basePath` prefija los hrefs internos (raíz con basePath='' → idéntico).
export async function RankingsBody({ ctx }: { ctx: PageContext }) {
  ...
  return (
    <DirectionalTransition>
          ...
          <div className="lpt-card" style={{ overflow: 'hidden' }}>
            ...
            {rankedWithRanks.map((player) => {
              ...
              return (
                <Link
                  key={player.id}
                  href={`${ctx.basePath}/players/${player.id}`}
      ...
      {unranked.length > 0 && (
        <section className="section">
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {unranked.map((p) => (
              <Link key={p.id} href={`${ctx.basePath}/players/${p.id}`} transitionTypes={['nav-forward']} className="lpt-card" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px' }}>
      ...
      <section className="section" style={{ display: 'flex', gap: 20, flexWrap: 'wrap' }}>
        <Link href={`${ctx.basePath}/rankings/pairs`} className="sec-link"><Users size={14} /> Ranking de parejas →</Link>
        <Link href={`${ctx.basePath}/rankings/tokens`} className="sec-link"><Coins size={14} /> Clasificación de La Timba →</Link>
      </section>
      </div>
    </DirectionalTransition>
  );
