...
⚠️ NO declarar `.default()` en las columnas nuevas de `groups` y NO usarlas en inserts existentes: son opcionales (NULL). El schema Drizzle solo puede declararlas cuando la columna física existe — en e2e la crea el global-setup (Step 6) y en prod el curl de la migración ANTES del deploy que las lea. Orden de deploy documentado en la PR: (1) mergear, (2) curl migrate-branding. Entre (1) y (2) las lecturas de `groups` enumerarían columnas inexistentes → para deploy ventana-cero, replicar el patrón 1A: **este plan mergea todo junto pero el usuario ejecuta el curl inmediatamente tras el deploy**; el riesgo real es minúsculo (solo rompería `/g/*` y el conmutador durante segundos; la raíz de Lomeros no lee groups en caliente… en realidad sí vía resolvePageContext). Aceptado por ser ventana de segundos, igual que en migraciones previas del repo (timba-v2, planner).
