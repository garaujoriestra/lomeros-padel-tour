...
| **1B-1 Jugadores/perfil/rankings** | `lib/players/queries.ts` scopeado; migrar `api/players*`, páginas de jugadores, rankings (elo/parejas/tokens), `profile-data.ts`. Primera aserción de no-fuga (un grupo no ve players del otro). | players, ratingHistory, pairStats, tokenLedger | Bajo |
| **1B-2 Partidos + rating + logros** | `lib/matches/queries.ts`; threading de `groupId` en `process-match.ts` (elo, pairStats, ratingHistory, achievements); `api/matches*`, páginas de partidos. | matches, matchSets, ratingHistory, pairStats, playerAchievements | Medio |
